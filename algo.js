...
function isNumber(row, col) {
	...
	let no = el.value;
	if (no >= 0 && no <= 9) {
		sudoku(row, col, no, el);
	} else {
		el.value = "";
	}
}

function sudoku(row, col, val, me) {
	white(me);
	// row
	for (let i = 0; i < 9; i++) {
		let r = parseInt((parseInt(row) + i) % 9);
		let el = document.getElementById("a" + r + col);
		if (el.value === val) {
			red(me); // pro max
		}
	}
	...
	// col
	for (let i = 0; i < 9; i++) {
		let c = parseInt((parseInt(col) + i) % 9);
		let el = document.getElementById("a" + row + c);
		if (el.value === val) {
			red(me); // pro max
		}
	}
	...
	// box
	let starterRow = parseInt(row) / 3;
	let starterCol = parseInt(col) / 3;

	starterRow = starterRow * 3;
	starterCol = starterCol * 3;

	for (let i = 0; i < 3; i++) {
		let r = parseInt(starterRow) + parseInt(i);
		for (let j = 0; j < 3; j++) {
			let c = parseInt(starterCol) + parseInt(j);
